...
class ExtremeLowModel(ModelPort):
    """
    XGBoost quantile (α=0.10) biased toward the lower tail.

    Heavy sample weights on the bottom *extreme_percentile* % of prices.
    Used in the ensemble during solar midday and overnight conditions to
    capture negative and near-zero prices.
    """
    ...
    def _compute_weights(self, y: np.ndarray) -> np.ndarray:
        ...
        threshold = np.percentile(y, self._extreme_percentile)
        dip_mask = y <= threshold
        if dip_mask.any():
            bottom = y.min()
            weights[dip_mask] = 1 + (self._max_weight - 1) * (
                (threshold - y[dip_mask]) / (threshold - bottom + 1e-8)
            )
        return weights
    ...
    def train(
        ...
        if X_val is not None and y_val is not None:
            ...
            logger.info("  ExtremeLow MAE=%.4f", self._metrics["mae"])
    ...
    def predict(self, X: pd.DataFrame) -> pd.DataFrame:
        if not self._trained or self._model is None:
            raise ModelNotTrainedError("ExtremeLowModel not trained")
