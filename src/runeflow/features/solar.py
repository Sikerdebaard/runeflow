...
class SolarPowerFeatures(FeatureGroup):
    ...
    def produces(self) -> tuple[str, ...]:
        return (
            "solar_power_output", "solar_power_6h", "solar_power_24h",
            "solar_ramp_rate", "solar_ramp_down", "solar_midday_surplus",
            "solar_scarcity", "clear_sky_index", "direct_diffuse_ratio",
            "solar_output_12h", "is_sunny_period",
        )
    ...
    def transform(self, df: pd.DataFrame, zone_cfg: ZoneConfig) -> pd.DataFrame:
        ...
        # Midday surplus
        is_midday = df.index.hour.isin([10, 11, 12, 13, 14, 15]).astype(int)
        ...
        if radiation_cols:
            df["clear_sky_index"] = (
                df[radiation_cols[0]] / (df["clear_sky_ghi"] + 1.0)
            ).clip(0, 1.5)
        elif cloud_cols:
        ...
        if direct_cols and diffuse_cols:
            df["direct_diffuse_ratio"] = (
                df[direct_cols[0]] / (df[diffuse_cols[0]] + 1.0)
            ).clip(0, 10.0)
