...
class CloudRadiationFeatures(FeatureGroup):
    ...
    def transform(self, df: pd.DataFrame, zone_cfg: ZoneConfig) -> pd.DataFrame:
        ...
        solar_cols = [
            ...
            if "radiation" in c.lower() or "solar" in c.lower()
        ]

        if cloud_cols:
            cloud_col = cloud_cols[0]
            df["cloud_change_rate"] = (
                df[cloud_col].diff(1).abs().rolling(3, min_periods=2).mean()
            )
            df["cloud_duration"] = (
                (df[cloud_col] > 70).astype(int).rolling(24, min_periods=1).sum()
            )
